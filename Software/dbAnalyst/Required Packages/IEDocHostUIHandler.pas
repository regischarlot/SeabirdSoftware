...
function TDocHostUIHandler.IDocHostUIHandler_FilterDataObject(const pDO: IDataObject; out ppDORet: IDataObject): HRESULT;
begin
  ppDORet := nil;
  Result := S_FALSE;
end;

function TDocHostUIHandler.IDocHostUIHandler_GetDropTarget(const pDropTarget: IDropTarget; out ppDropTarget: IDropTarget): HRESULT;
begin
  Result := S_FALSE;
end;
...
function TDocHostUIHandler.IDocHostUIHandler_GetOptionKeyPath(var pchKey: POLESTR; const dw: DWORD): HRESULT;
begin
  Result := S_FALSE;
end;
...
function TDocHostUIHandler.IDocHostUIHandler_OnFrameWindowActivate( const fActivate: BOOL): HRESULT;
begin
  Result := S_FALSE;
end;

function TDocHostUIHandler.IDocHostUIHandler_ResizeBorder(const prcBorder: PRECT; const pUIWindow: IOleInPlaceUIWindow; const fRameWindow: BOOL): HRESULT;
begin
  Result := S_FALSE;
end;

function TDocHostUIHandler.IDocHostUIHandler_ShowContextMenu(const dwID: DWORD; const ppt: PPOINT; const pcmdtReserved: IUnknown; const pdispReserved: IDispatch): HRESULT;
begin
...
function TDocHostUIHandler.IDocHostUIHandler_ShowUI(const dwID: DWORD; const pActiveObject: IOleInPlaceActiveObject; const pCommandTarget: IOleCommandTarget; const pFrame: IOleInPlaceFrame; const pDoc: IOleInPlaceUIWindow): HRESULT;
begin
  Result := S_FALSE;
end;
...
function TDocHostUIHandler.IDocHostUIHandler_TranslateUrl(const dwTranslate: DWORD; const pchURLIn: POLESTR; var ppchURLOut: POLESTR): HRESULT;
begin
  Result := S_FALSE;
end;
...
initialization
  TComObjectFactory.Create(ComServer, TDocHostUIHandler, Class_DocHostUIHandler, 'DocHostUIHandler', 'Implements DocHostUIHandler', ciMultiInstance);
end.
